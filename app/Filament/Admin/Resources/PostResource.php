...
class PostResource extends Resource
{
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                ImageColumn::make('image')->label('Миниатюра'),
                TextColumn::make('title')->label('Название поста')->sortable()->searchable(),
                TextColumn::make('slug')->label('URL')->sortable()->searchable(),
                TextColumn::make('author.name')->label('Автор')->sortable()->searchable(),
                TextColumn::make('published_at')->label('Время публикации')->date('Y-m-d')->sortable()->searchable(),
                CheckboxColumn::make('featured')->label('Рекомендации'),
            ])
            ->filters([
